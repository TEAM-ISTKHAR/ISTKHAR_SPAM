...
def load_plugins(plugin_name):
    path = Path(f"ISTKHAR/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"ISTKHAR.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["ISTKHAR.modules." + plugin_name] = load
    print("Altron has Imported " + plugin_name)


files = glob.glob("ISTKHAR/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\n𝗜𝘀𝘁𝗸𝗵𝗮𝗿 𝐒𝐩𝐚𝐦 𝐁𝐨𝐭𝐬 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 ⚡\nMy Master ---> @ll_THUNDER_lll")
